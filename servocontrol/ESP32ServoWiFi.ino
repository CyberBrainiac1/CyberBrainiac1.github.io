...
constexpr char kAccessPointSSID[] = "ESP32-Servo-Remote";
constexpr char kAccessPointPassword[] = "servo1234";
constexpr char kHostname[] = "servo-control";
...
bool credentialsConfigured() {
  return String(ServoRemote::kDefaultStationSSID) != "YOUR_WIFI_NAME" &&
         String(ServoRemote::kDefaultStationPassword) != "YOUR_WIFI_PASSWORD" &&
         strlen(ServoRemote::kDefaultStationSSID) > 0;
}
...
String activeAddress() {
  return networkMode == NetworkMode::station
...
void handleRoot() {
  ...
  server.send(
      200,
      "application/json",
      "{\"message\":\"ESP32 servo remote is running\",\"status\":\"ok\"}");
}
...
void startAccessPoint() {
  ...
  Serial.printf("Started fallback access point \"%s\"\n", ServoRemote::kAccessPointSSID);
...
void setup() {
  ...
  configureHttpServer();
  Serial.println("ESP32 Servo Wi-Fi Remote is ready");
}
